...
from process_batch import get_review_and_insert, get_result_data
...
st.markdown(
    "<h1 style='text-align: center;'>📚 Playstore Sentiment Analysis App</h1>",
    unsafe_allow_html=True,
)
st.write(
    "Aplikasi sederhana untuk melakukan analisis sentimen terhadap sentiment dari google playstore"
)
...
submit = st.button("🔍Cari Review")


if submit:

    with st.spinner("Loading Model... (1/3)"):

        project_id = "hvzn-development"
        preds = SentimentPredictor()

    with st.spinner("Mengambil data dari playstore... (2/3)"):
        ...
        app_mapping = {
            ...
            "BTN": "id.co.btn.mobilebanking.android",
            "JAGO": "com.jago.digitalBanking",
        }

        app = app_mapping.get(option_bank, None)

        if app:
            get_review_and_insert(
                app_id=app, engine=preds.db_engine, conn=preds.db_conn
            )

    with st.spinner("Melakukan prediksi sentiment... (3/3)"):
        preds.batch_prediction(batch_size=1000)

    # get data from postgressql
    df = get_result_data(app, "2022-01-01", preds.db_engine)
    df = df[["at", "content", "sentiment"]]
...
#         "Pilih Model",
#         (
#             "IndoBERT (Accurate,Slow)",
#             "Naive Bayes",
#             "Logistic Regression (Less Accurate,Fast)",
#             "XGBoost",
#             "Catboost",
#             "SVM",
#             "Random Forest",
...
#         length = st.number_input("Jumlah Tweet", 10, 500, 100)
#     st.caption(
#         "anda bisa menggunakan parameter pencarian yang lebih spesifik, parameter ini sama dengan paremeter pencarian di twitter"
#     )
